...
def build_unet(crop_size, img_c):
    ...
    norm = layers.Lambda(lambda x: x / 255)(inputs) # Normalize the pixels
    
    # Encoder
    c1 = contraction(16, 0.1, norm)
    p1 = pooling(c1)
    
    c2 = contraction(32, 0.1, p1)
    p2 = pooling(c2)
    
    c3 = contraction(64, 0.2, p2)
    p3 = pooling(c3)
    
    c4 = contraction(128, 0.2, p3)
    p4 = pooling(c4)
    
    # Bottom of U
    c5 = contraction(256, None, p4)
    
    # Decoder
    e4 = expansion(128, 0.2, c5, concat_layer=c4)
    
    e3 = expansion(64, 0.2, e4, concat_layer=c3)
    
    e2 = expansion(32, 0.1, e3, concat_layer=c2)
    
    e1 = expansion(16, 0.1, e2, concat_layer=c1)
